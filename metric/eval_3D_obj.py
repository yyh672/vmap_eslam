...
def calc_3d_metric(mesh_rec, mesh_gt, N=200000):
    ...
    transform, extents = trimesh.bounds.oriented_bounds(mesh_gt)#有向包围盒
    extents = extents / 0.9  # enlarge 0.9
    box = trimesh.creation.box(extents=extents, transform=np.linalg.inv(transform))#立方体的三维网格模型
    mesh_rec = mesh_rec.slice_plane(box.facets_origin, -box.facets_normal)#将原始模型切割成两部分，即切片平面两侧的部分和切片平面上的部分
    ...
    accuracy_rec = accuracy(gt_pc_tri.vertices, rec_pc_tri.vertices)#传进去的还是三维坐标
