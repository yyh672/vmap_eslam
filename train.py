    ...
    for frame_id in tqdm(range(dataset_len)):
                ...
                for obj_id in obj_ids:
                    ...
                    if obj_id in vis_dict.keys():
                        ...
                        if cfg.do_bg and obj_id == 0:   # todo param, Here we use ESLAM
                            ...
                            #double optimiser
                            decoders_para_list = []
                            decoders_para_list += list(scene_bg.trainer.decoders.parameters())
                            planes_para = []
                            for planes in [scene_bg.trainer.eslam.shared_planes_xy, scene_bg.trainer.eslam.shared_planes_xz, scene_bg.trainer.eslam.shared_planes_yz]:
                            ...
                            c_planes_para = []
                            for c_planes in [scene_bg.trainer.eslam.shared_c_planes_xy,scene_bg.trainer.eslam.shared_c_planes_xz, scene_bg.trainer.eslam.shared_c_planes_yz]:
                                for i, c_plane in enumerate(c_planes):
                                    c_plane = nn.Parameter(c_plane)
                                    c_planes_para.append(c_plane)
                                    c_planes[i] = c_plane
                            bg_optimiser = torch.optim.Adam([{'params': decoders_para_list, 'lr': 0},
        ...
        with performance_measure(f"Sampling over {len(obj_dict.keys())} objects,"):
            if cfg.do_bg and scene_bg is not None:
                ...
                #200*120*3->24000*3
                #np.savetxt("origins.txt", bg_origins.cpu().numpy())
                #np.savetxt("bg_dirs_W.txt", bg_dirs_W.cpu().numpy())
                #np.savetxt("bg_gt_depth.txt",bg_gt_depth.cpu().numpy())
                
            for obj_id, obj_k in obj_dict.items():
                #input_pcs 相机信息
                gt_rgb, gt_depth, valid_depth_mask, obj_mask, input_pcs, sampled_z, obj_origin,obj_dirs\
        ...
        with performance_measure(f"Training over {len(obj_dict.keys())} objects,"):
            ...
            with performance_measure(f"Batch LOSS"):
                ...
                if cfg.do_bg:                   
                    #eslam loss
                    # SDF losses
                    bg_valid_depth_mask = bg_valid_depth_mask.reshape(bg_gt_depth.shape[0])
                    bg_obj_mask = bg_obj_mask.reshape(bg_gt_rgb.shape[0])

                    mask_obj = bg_obj_mask == 1
